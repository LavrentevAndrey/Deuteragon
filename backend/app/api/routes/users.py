...
def delete_user_me(session: SessionDep, current_user: CurrentUser) -> Any:
    ...
    statement = delete(Device).where(col(Device.owner_id) == current_user.id)
...
def delete_user(
    ...
    statement = delete(Device).where(col(Device.owner_id) == user_id)
    session.exec(statement)  # type: ignore
    session.delete(user)
    session.commit()
    return Message(message="User deleted successfully")
...
# TODO: Add async routes for async DB
# from fastapi import APIRouter, Depends, HTTPException
